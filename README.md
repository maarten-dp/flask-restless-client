...
beast.save()
```
Note that if we disregard the `c.save()` statement, and run `beast.save()` instead, that the `maurice` instance will be unsaved at the time we call `beast.save()`
...
# filtering over relations, get all persons that own a computer with Pear vendor
maurice = c.Person.query.filter(c.Person.computers.vendor == 'Pear')
```
