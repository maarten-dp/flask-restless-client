...
def test_it_can_set_a_remote_property(mcl):
    ...
    mcl.save(apt)
    assert not inspect(apt).dirty
    mcl.registry = {}
    assert mcl.Apartment.query.one().settable_property == "ApSetMent"


def test_it_casts_attribute_types_correctly_when_not_marshalling_from_json(mcl):
    apt = mcl.Apartment.query.one()
    mt = apt.function_with_new_obj()

    expected_types = {"date": date, "dt": datetime, "json": dict, "boolean": bool}
